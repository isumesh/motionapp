...
class FormularioRiesgoScreen extends Component<Props> {
  ...
  onSetSimpleInput = (value, pregunta) => {
    const {
      respuestas,
      actions: { obtenerPreguntasFormRiesgo },
    } = this.props;
    const { form } = this.state;
  ...
  onCreateSummaryRiesgo = () => {
    ...
    const plataformaCreador =
      currentUser.plataforma || form["plataformaCreador"];
  ...
  renderLibreInput = (key, pID) => {
    ...
    return (
      <View key={key}>
        ...
        <Text style={maxLengthStyle}>{`${_.size(
          respuesta
        )}/${maxLength}`}</Text>
        {requeridoFaltante ? (
          <Text style={styles.requeridoMssg}>campo requerido</Text>
      ...
      <View style={styles.inputContainer}>
        {Platform.OS === "ios" ? (
          <Modal
            animationType={"slide"}
            transparent={true}
            visible={riesgoDateModalVisible}
        ...
        >
          {riesgoDate ? (
            <Text style={styles.inputTitle}>Fecha riesgo*:</Text>
          ) : null}
          <View style={styles.itemContainer}>
            <Text style={styles.inputText}>
              {riesgoDate
                ? riesgoDate.format("dddd, DD MMMM YYYY")
  ...
  renderRutClienteInput = () => {
    ...
    ].concat(requeridoFaltante ? styles.inputContainerRequired : []);
    const inputTitleStyle = [styles.inputTitle].concat(
      requeridoFaltante ? styles.inputTitleRequired : []
    );
    const inputTextStyle = [styles.inputText].concat(
    ...
    return (
      <View style={styles.container}>
        <ScrollView style={styles.formContainer}>
          ...
          <View
            style={
              rutCliente ? styles.inputContainer : styles.inputDisabledContainer
            }
          >
            {nombreCliente ? (
        ...
        <TouchableOpacity
          onPress={() => {
            !isFetchingObtenerPregunta && this.validationForm(true)
              ? this.onCreateSummaryRiesgo()
              : false;
          }}
          style={
            !isFetchingObtenerPregunta && this.validationForm()
