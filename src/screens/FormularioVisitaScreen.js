...
import { idsArrayParse } from "../utils/ArrayHelpers";
import {
  desformatoRut,
  formatoRut,
  invalidDecimal,
...
const mapStateToProps = (state, ownProps) => {
  ...
  return {
    ...
    currentUser:
      API_ENV === "dev" ? state.currentUser.profile : state.auth.profile,
...
class FormularioVisitaScreen extends Component<Props> {
  static navigationOptions = ({ navigation, screenProps }) => ({
    headerTitle: `${
      navigation.getParam("visitaId") ? "Edición de" : "Nueva"
    } visita`,
    headerLeft: (
      <GoBackButton
        onPress={() => {
  ...
  componentWillMount() {
    ...
    const participante = _.find(
      colaboradores,
      (col) => col.usuarioNt === usuario
    ) || {
      ...
      usuarioNtJefe:
        _.find(colaboradores, (col) => col.usuarioNt === currentUser.usuario) ||
        "",
    };

    const { nombreColaborador, usuarioNt, centroCosto, usuarioNtJefe } =
      participante;
  ...
  componentWillReceiveProps(nextProps) {
      ...
      this.setState({
        form: {
          ...
          ...(nextProps.hasErrorEmpresa
            ...
            : {
                macroBanca: _.capitalize(
                  _.first(_.values(nextProps.empresas)).macroBanca
                ),
                clienteProspecto: false,
              }),
        },
    ...
    if (visitaId && isFetching && !nextProps.isFetching) {
      ...
      console.log("detalles", detallesCargados);
      console.log("nextProps", nextProps.visita.detalle);
      if (visitaId && detallesCargados < _.size(nextProps.visita.detalle)) {
        const respuestaId =
          nextProps.visita.detalle[detallesCargados].respuestaId;
        console.log("respuestaId", respuestaId);
        respuestaId
          ? obtenerPreguntasFormVisita({ respuestaId })
          : this.setState({ detallesCargados: detallesCargados });
      } else {
        const tabKey = _.find(
          _.keys(nextProps.preguntas),
          (k) => nextProps.preguntas[k].tipo === TipoPregunta.TABS
        );
        const tabTitle = nextProps.preguntas[tabKey].name;
        const firstTabValue = _.first(_.keys(nextProps.respuestas[tabKey]));
        const firstTabName = _.first(
          _.values(nextProps.respuestas[tabKey]) || {}
        ).name;
      ...
      initialFormLoaded &&
      isFetchingObtenerPregunta &&
      !nextProps.isFetchingObtenerPregunta
    ) {
    ...
    if (
      !_.isEmpty(nextProps.visita) &&
      isValidatingEmpresa &&
      !nextProps.isValidatingEmpresa
    ) {
  ...
  componentWillUpdate(nextProps, nextState) {
    const {
      actions: { obtenerEmpresas },
    } = this.props;
    const { visitaId } = nextProps;
    const {
      form,
      form: { rutCliente, riesgos },
    } = this.state;
    ...
    const siguienteSubdetalleVisita =
      nextState.form["Subdetalle de la visitaNombre"];

    if (visitaId && !_.isEqual(riesgos, nextState.form.riesgos)) {
      this.validarOtrosRiesgosCreados(
        siguienteRutCliente,
        siguienteMotivoVisita,
        siguienteSubdetalleVisita
      );
    } else {
      if (siguienteRutCliente && siguienteMotivoVisita) {
        if (
          rutCliente !== siguienteRutCliente ||
          motivoVisita !== siguienteMotivoVisita ||
          subdetalleVisita !== siguienteSubdetalleVisita
        ) {
          this.validarOtrosRiesgosCreados(
            siguienteRutCliente,
            siguienteMotivoVisita,
  ...
  componentDidUpdate(prevProps, prevState) {
    const {
      actions: { obtenerPreguntasFormVisita },
      initialFormLoaded,
      visita: { detalle },
    } = this.props;
    const {
      loading,
      saving,
      form: { oportunidades, riesgos },
      resumen: { oportunidadesId, riesgosId },
      detallesCargados,
    } = this.state;
    ...
    if (loading) {
      ...
      if (visitaId && initialFormLoaded) {
        ...
        if (!_.isEmpty(preguntas) && _.size(preguntas) === _.size(detalle)) {
          ...
          prevState.detallesCargados < detallesCargados &&
          detallesCargados < _.size(detalle)
        ) {
      ...
      const nuevaOportunidadCreada =
        _.size(oportunidadesId) > _.size(prevState.resumen.oportunidadesId);
      const nuevoRiesgoCreado =
        _.size(riesgosId) > _.size(prevState.resumen.riesgosId);
      const sinOportunidadesPorCrear =
        _.size(oportunidadesId) === _.size(oportunidades);
      ...
      if (nuevaOportunidadCreada || nuevoRiesgoCreado) {
        if (sinOportunidadesPorCrear && sinRiesgosPorCrear) {
          ...
          if (nuevaOportunidadCreada && !sinOportunidadesPorCrear)
            this.onSaveOportunidades();
  ...
  setVisitaFormEdition = (visita) => {
    ...
    const {
      ...
      detalle,
      resumen: { nombreEmpresa, rutEmpresa, grupoEconomico, fechaVisita },
      oportunidadesId,
      riesgosId,
      privado,
      usuarioNTCreador,
    ...
    const jefeNTCreador = _.find(
      colaboradores,
      (col) => col.usuarioNt === currentUser.usuario
    );
    if (oportunidadesId)
      obtenerOportunidadesPorIds({
  ...
  goToFormularioRiesgo = () => {
    ...
    const {
      form: { rutCliente, nombreCliente, grupoEconomico, macrobancaEmpresa },
    } = this.state;
    let clienteVisita;

    if (grupoEconomico && !rutCliente) {
      const cliente = _.find(
        empresas,
        (ep) => _.lowerCase(ep.nombreGrupo) === _.lowerCase(grupoEconomico)
      );
    ...
    navigate("FormularioRiesgo", {
      fromVisita: true,
      clienteVisita,
      addToVisita: this.onSetRiesgo,
    });
  };
  ...
  goToFormularioOportunidad = () => {
    ...
    navigate("FormularioOportunidad", {
      fromVisita: true,
      clienteVisita,
      addToVisita: this.onSetOportunidad,
    });
  };
  ...
  onSetEmpresaGrupoEconomico = (value) => {
    const { rut, digitoVerificador, nombreEmpresa, nombreGrupo, macroBanca } =
      value;
  ...
  onSetParticipante = (value, i) => {
    ...
    if (
      !_.find(participantes, (pt) => pt.usuarioNT === participante.usuarioNT)
    ) {
  ...
  onSetOportunidad = (oportunidad) => {
    ...
    this.setState({
      form: { ...form, oportunidades: _.concat(oportunidades, oportunidad) },
    });
  };
    ...
    switch (motivoVisita) {
          ...
          case "Cierre de Negocio":
            ...
            if (
              !_.find(
                riesgos,
                (riesgo) =>
                  riesgo.usuarioNTCreador === currentUser.usuario.trim()
              )
            ) {
              return limitDays;
            } else {
              const riesgosDeUsuario = _.filter(
                riesgos,
                (riesgo) =>
                  riesgo.usuarioNTCreador === currentUser.usuario.trim()
              );
              ...
              if (
                !!_.find(
                  ...
                  (riesgo) =>
                    new Date(
                      moment(riesgo.fechaRiesgo).add(limitDays, "days").format()
                    ) < today
                )
              ) {
                return limitDays;
      ...
      case "Riesgo":
        ...
        if (
          !_.find(
            riesgos,
            (riesgo) => riesgo.usuarioNTCreador === currentUser.usuario.trim()
          )
        ) {
          return limitDays;
        } else {
  ...
  validarOtrosRiesgosCreados = (rutCliente, motivoVisita, subdetalleVisita) => {
    const {
      actions: { obtenerVigenciaRiesgos },
      currentUser,
    } = this.props;
  ...
  puedeCrearRiesgo = () => {
    ...
    const {
      currentUser: {
        permisos: { puedeCrearRiesgo },
      },
    } = this.props;
    // TODO cambiar el ambiente android -> produccion ios -> qa SSL
    // return (API_ENV !== 'prodApiConnect') ||
    return (
      API_ENV !== "qa_Azure" ||
      (puedeCrearRiesgo &&
  ...
  validationForm = (validarRequeridos = false) => {
    ...
    if (
      !existenRiesgosVigentes &&
      this.puedeCrearRiesgo() &&
      this.validarRiesgosRequeridos()
    ) {
      validation++;
    }
  ...
  renderAlertModal = () => {
    ...
    return (
      ...
      >
        <Text style={styles.modalTitle}>Visita guardada exitosamente</Text>
        ...
        <View style={styles.modalContainerButtons}>
          ...
          <TouchableOpacity
            onPress={() => this.goNuevaVisita()}
            style={[styles.buttonModal, styles.buttonSolid]}
          >
            <Text style={styles.textModalSolidButton}>Nueva visita</Text>
          </TouchableOpacity>
        </View>
      </ModalBox>
    );
  ...
  goNuevaVisita = () => {
    ...
    replace("FormularioVisita", { fromMain, tipoVisita });
  };
  ...
  onCreateSummaryVisita = () => {
    ...
    const plataformEmpresa =
      plataform === undefined || plataform == "" ? null : plataform;

    const usuarioNTParticipantes = _.map(
      _.filter(form["participantes"], (pt) => !!pt.usuarioNT),
      (pt) => {
        return {
    ...
    Object.keys(form).map((st) => {
      Object.keys(preguntas || {}).map((key) => {
        ...
        if (st === preguntas[key].name) {
          ...
          if (st === "Motivo de la visita")
            Object.assign(resumen, { tituloVisita: form[`${st}Nombre`] });
          ...
          detalle.push({
            ...
            ...(tipo === LIBRE
              ...
              : {
                  respuesta: form[`${st}Nombre`],
                  respuestaId: Number(form[st]),
                }),
          });
        }
      });
    ...
    //TODO Esta linea es la que funciona bien

    Object.assign(paramsForm, {
      jefeNTCreador: visitaId
        ? this.props.visita.jefeNTCreador
        : jefeNTCreador.usuarioNtJefe,
    });
    // Object.assign(paramsForm, {jefeNTCreador: jefeNTCreador.usuarioNtJefe})

    Object.assign(resumen, {
      usuarioNombreParticipantes: _.compact(
        _.map(
          form["participantes"],
          (participante) => _.trim(participante.usuarioNombre) || null
        )
      ),
    });
  ...
  onSaveVisita = () => {
    const {
      actions: { crearVisita, actualizarVisita },
      visitaId,
    } = this.props;
  ...
  guardarVisita = () => {
    ...
    const oportunidadesPorCrear =
      _.size(oportunidades) > _.size(oportunidadesId);
    ...
    return (
      <View key={i} style={inputStyle}>
        <Modal
          animationType={"slide"}
          transparent={false}
          visible={participanteModalVisible}
          onRequestClose={() => {}}
        >
          <SearchModal
            title="Participante"
        ...
        <TouchableOpacity
          onPress={() =>
            visitaId ? null : this.setState({ [modalName]: true })
          }
        >
          <Text style={styles.inputTitle}>Participante visita:</Text>
  ...
  renderSimpleInput = (key, pID) => {
    ...
    const requeridoFaltante =
      obligatorio && requeridosFaltantes && !actualRespuesta;
    const inputStyle = [styles.inputContainer].concat(
      requeridoFaltante ? styles.inputContainerRequired : []
    );
    const inputTitleStyle = [styles.inputTitle].concat(
      requeridoFaltante ? styles.inputTitleRequired : []
    );
    const inputTextStyle = [styles.inputText].concat(
      requeridoFaltante ? styles.inputTextRequired : []
    );
    ...
    if (tipo === TipoPregunta.SIMPLE) {
      ...
      return (
        <View key={key}>
          <View style={inputStyle}>
            ...
            >
              <ItemListModal
                list={respuestas}
                saveSelection={(value) =>
                  this.onSetSimpleInput(value, { ...pregunta, id: key })
                }
                title={name}
                pID={pID}
                headerLeft={
            ...
            <TouchableOpacity
              onPress={() =>
                visitaId ? null : this.setState({ [tituloModal]: true })
              }
            >
              {actualRespuestaNombre ? (
                <Text style={inputTitleStyle}>{`${tituloInput}:`}</Text>
              ) : null}
              <View style={styles.itemContainer}>
          ...
          {requeridoFaltante ? (
            <Text style={styles.requeridoMssg}>campo requerido</Text>
          ) : null}
        </View>
      );
    } else {
      ...
      return (
        <View
          key={key}
          style={[{ marginBottom: 30 }].concat(
            visitaId ? [styles.inputContainerDisabled] : []
          )}
        >
  ...
  renderLibreInput = (key, pID) => {
    ...
    return (
      <View key={key}>
        <View style={inputStyle}>
          <Text style={inputTitleStyle}>{`${name}${
            obligatorio ? "*" : ":"
          }`}</Text>
          <InputGroup
            editable={!visitaId}
      ...
      <View style={inputStyle}>
        {Platform.OS === "ios" ? (
          <Modal
            animationType={"slide"}
            transparent={true}
            visible={visitaDateModalVisible}
        ...
        <TouchableOpacity
          onPress={() => {
            visitaId ? null : this.setState({ visitaDateModalVisible: true });
          }}
        >
          {visitaDate ? (
            <Text style={styles.inputTitle}>Fecha visita*:</Text>
          ) : null}
          <View style={styles.itemContainer}>
            <Text style={styles.inputText}>
              {visitaDate
                ? visitaDate.format("dddd, DD MMMM YYYY")
  ...
  renderRutClienteInput = () => {
    const { visitaId } = this.props;
    const {
      rutClienteModalVisible,
      form: { rutCliente, grupoEconomico },
      requeridosFaltantes,
    } = this.state;
      ...
      <View>
        <View style={inputStyle}>
          <Modal
            animationType={"slide"}
            transparent={false}
            visible={rutClienteModalVisible}
            onRequestClose={() => {}}
          >
            <SearchModal
              title="RUT Cliente"
          ...
          <TouchableOpacity
            onPress={() =>
              visitaId ? null : this.setState({ rutClienteModalVisible: true })
            }
          >
            {rutCliente ? (
              <Text style={inputTitleStyle}>RUT cliente*:</Text>
            ) : null}
            <View style={styles.itemContainer}>
              <Text style={inputTextStyle}>
                {rutCliente
                  ? formatoRut(rutCliente)
  ...
  renderGrupoEconomicoInput = () => {
    const { visitaId } = this.props;
    const {
      grupoEconomicoModalVisible,
      form: { grupoEconomico, rutCliente },
      requeridosFaltantes,
    } = this.state;
    const obligatorio = !rutCliente;
    const requeridoFaltante =
      obligatorio && requeridosFaltantes && !grupoEconomico;
      ...
      <View>
        <View style={inputStyle}>
          <Modal
            animationType={"slide"}
            transparent={false}
            visible={grupoEconomicoModalVisible}
            onRequestClose={() => {}}
          >
            <SearchModal
          ...
          <TouchableOpacity
            onPress={() =>
              visitaId
                ? null
                : this.setState({ grupoEconomicoModalVisible: true })
            }
          >
            {grupoEconomico ? (
              <Text style={inputTitleStyle}>Grupo económico*:</Text>
            ) : null}
            <View style={styles.itemContainer}>
              <Text style={inputTextStyle}>
                {grupoEconomico
                  ? grupoEconomico
                  : `Grupo económico${obligatorio ? "*" : ":"}`}
              </Text>
              <Image
                style={{ marginRight: 10 }}
                source={require("./../../assets/images/icons/down_arrow_icon.png")}
  ...
  renderResumenVisita = () => {
    ...
    const resumen = {
      ...
      plataform,
      fechaCreacion:
        this.state.resumen.resumen !== undefined
          ? this.state.resumen.resumen.fechaCreacion
          : fechaVisita,
    };
    ...
    return (
      <Modal
        animationType={"slide"}
        transparent={false}
        visible={resumenModalVisible}
        onRequestClose={() => {}}
      >
        <View
          style={{ flex: 1, backgroundColor: BACKGROUND_COLOR, paddingTop: 15 }}
        >
          <NavBar
  ...
  renderNotaInput = (nota, key) => {
    ...
    if (fechaCreacion.length === 0) {
      ...
      let fechaCreacionOportunidad = moment(
        fechaCreacion,
        "YYYY-MM-DD HH:mm:ss Z"
      );
    ...
    return (
      <View key={key}>
        ...
        >
          <Text style={styles.horaNotaOportunidad}>
            {usuarioCreador} {horaNota}
          </Text>
        </View>
        <View
          style={[styles.inputContainer, styles.notaInputContainer].concat(
        ...
        >
          <Text style={styles.maxLength}>{`${_.size(
            contenido
          )}/${maxLength}`}</Text>
        </View>
      </View>
  ...
  renderForm = () => {
    ...
    const riesgoRequerido =
      !existenRiesgosVigentes && this.validarRiesgosRequeridos();
        ...
        >
          <ScrollView style={styles.formContainer}>
            <View style={styles.userContainer}>
              ...
              <Text style={styles.creator}>{nombreCreador}</Text>
              <Text style={styles.dateHeader}>{fechaVisita}</Text>
            </View>
            ...
            {_.map(llavesDependencias, (key, i) => {
              ...
              if (preguntas[key].main)
                parentInput = this.renderFormInput(preguntas[key], key, i + 1);
              ...
              return parentInput ? (
                <View key={key}>
                  ...
                  {_.map(preguntas[key].childrensId, (childrenKey) => {
                    j += 1;
                    const childrenFormInput = this.renderFormInput(
                      preguntas[childrenKey],
                      childrenKey,
                      j
                    );
            ...
            <View style={{ marginBottom: 10, marginTop: 20 }}>
              ...
              {(puedeCrearRiesgo || !_.isEmpty(riesgos)) && (
                <Text
                  style={[styles.titleButton].concat(
                    riesgoRequeridaMssg ? { color: RED } : []
                  )}
                >
              ...
              {_.map(riesgos, (riesgo, i) => {
                ...
                const resumenRiesgoModalVisible =
                  !!this.state[resumenRiesgoModalName];
                ...
                return (
                  <View key={i}>
                    ...
                    <TouchableOpacity
                      onPress={() =>
                        this.obtenerResumenRiesgo(
                          resumen,
                          resumenRiesgoModalName
                        )
                      }
                    >
                    ...
                    <Modal
                      animationType={"slide"}
                      transparent={false}
                      visible={resumenRiesgoModalVisible}
                      onRequestClose={() => {}}
                    >
                      <View
                        style={{
                      ...
                      >
                        ...
                        <ScrollView>
                          {isFetchingRiesgos ||
                          (riesgoId &&
                            this.props.riesgo.riesgoId !== riesgoId) ? (
                            <Loading />
                          ) : (
              ...
              {puedeCrearRiesgo ? (
                <TouchableOpacity
                  onPress={this.goToFormularioRiesgo}
                  style={styles.button}
                >
                  <Image
                    style={{ marginRight: 10 }}
            ...
            {!visitaId || _.size(visita.oportunidadesId) ? (
              <View style={{ marginBottom: 20 }}>
                <Text style={styles.titleButton}>{`Oportunidades${":"}`}</Text>
                <View style={styles.contentRadioHideButtonOport}>
                ...
                {requeridosFaltantes === true && validateRadioButton ? (
                  <Text style={styles.requeridoMssg}>
                    Tienes que agregar una oportunidad a esta visita antes de
                    guardar.
                  </Text>
                ) : null}
                ...
                {_.map(oportunidades, (oportunidad, i) => {
                  ...
                  const resumen = oportunidadId
                    ? oportunidad
                    : {
                        ...oportunidad.resumen,
                        monto: `$ ${
                          oportunidad.monto
                            ? montoPuntosMil(oportunidad.monto)
                            : 0
                        }`,
                        nombreEstado: oportunidad.nombreEstado || "Activa",
                        fechaFin: oportunidad.fechaFin,
                      };
                  const resumenOportunidadModalName = [
                    `ResumenOportunidad[${i}]Visible`,
                  ];
                  const resumenOportunidadModalVisible =
                    !!this.state[resumenOportunidadModalName];
                  ...
                  if (oportunidad.mostrar) {
                    return (
                      <View key={i}>
                        <TouchableOpacity
                          onPress={() =>
                            this.obtenerResumenOportunidad(
                              resumen,
                              resumenOportunidadModalName
                            )
                          }
                        >
                          {this.state.hideCardOportunidad ? (
                            <OportunidadCard
                              oportunidad={resumen}
                              navigation={this.props.navigation}
                              fromVisita={true}
                        ...
                        <Modal
                          animationType={"slide"}
                          transparent={false}
                          visible={resumenOportunidadModalVisible}
                          onRequestClose={() => {}}
                        >
                          <View
                            style={{
                          ...
                          >
                            <NavBar
                              ...
                              headerLeft={
                                <GoBackButton
                                  onPress={() => {
                                    this.setState({
                                      [resumenOportunidadModalName]: false,
                                    });
                                  }}
                                />
                              }
                            ...
                            <ScrollView>
                              {isFetchingOportunidades ||
                              (oportunidadId &&
                                _.isEmpty(this.props.oportunidad)) ? (
                                <Loading />
                              ) : (
                                <ResumenOportunidadScreen
                                  title="Resumen Oportundidad"
                ...
                {visitaId ? null : this.state.isHidden ? (
                  <TouchableOpacity
            ...
            <ButtonAddNota addNota={this.agregarNota} title={"Agregar Nota"} />
          </ScrollView>
        </KeyboardAvoidingView>
        <TouchableOpacity
          onPress={() => {
          ...
          style={
            ...
            this.validationForm()
              ...
              : styles.buttonsDisabledContainer
          }
        >
