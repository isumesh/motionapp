...
import {
  getEstadoOportunidadActiva,
  getOportunidadesFiltradas,
...
class OportunidadesScreen extends Component<Props> {
  static navigationOptions = ({ navigation, screenProps }) => ({
    ...
    headerRight: (
      <FilterButton
        currentScreen={"OportunidadesScreen"}
        onPress={() => {
  ...
  componentWillMount() {
    ...
    const platformEmpr =
      plataformaEmpresa === undefined ? "" : plataformaEmpresa;
    clearOportunidad();
    obtenerListaOportunidad({
      usuarioNTResponsable,
      clienteId,
  ...
  getOportunidadesList = () => {
    ...
    if (_.isEmpty(oportunidades)) {
      return (
        <View>
          <Text>No hay oportunidades</Text>
        </View>
      );
    }
  ...
  siguientePagina = (e) => {
    ...
    const platformEmpr =
      this.props.filters.plataformaEmpresa === undefined
        ? ""
        : this.props.filters.plataformaEmpresa;
    const filters = {
    ...
    if (error === "unauthorized" && hasError === true) {
      ...
      return (
        <View style={styles.container}>
          <ScrollView
            style={{ marginTop: 5 }}
            onScroll={(e) => this.siguientePagina(e)}
          ...
          >
            <View style={styles.summaryOportunidad}>
              ...
              <Text style={styles.total}>
                {loading
                  ? "Cargando..."
                  : `$${formatearMonto(parseInt(montoOportunidades))}`}
              </Text>
            </View>

            {this.getOportunidadesList()}

            {isRefreshing ? <Loading /> : undefined}
          </ScrollView>
        </View>
      );
    }
