...
const mapStateToProps = (state, ownProps) => {
  ...
  return {
    ...
    error:
      state.formularioOportunidad.errorMessage ||
      state.oportunidades.errorMessage,
...
class FormularioOportunidadScreen extends Component<Props> {
  ...
  static navigationOptions = ({ navigation, screenProps }) => ({
    headerTitle: `${
      navigation.getParam("oportunidadId") ? "Edición de" : "Nueva"
    } oportunidad`,
    headerLeft: (
      <GoBackButton
        onPress={() => {
  ...
  componentWillMount() {
    ...
    if (oportunidadId) {
      ...
      const colaborador = _.find(
        colaboradores,
        (responsable) => responsable.usuarioNt === _.trim(currentUser.usuario)
      ) || {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (isFetching && !nextProps.isFetching) {
      ...
      if (nextProps.oportunidadId) {
        const {
          actions: { actualizarDetalleOportunidad },
        } = this.props;
        const {
          initFormLoaded,
          form: { estadoId },
          ...
          if (loadingMainData) {
            if (
              ...
              if (detalleSize < _.size(nextProps.backup.preguntas)) {
                const prgs = _.remove(
                  oportunidadDetalle,
                  (od) => !!nextProps.backup.preguntas[od.preguntaId]
                );
              ...
              _.map(
                _.slice(backupDetalle, backupDataLoaded, detalleSize),
                (d) => {
                  const { respuestaId } = d;
            ...
            if (
              ...
              _.map(preguntasKeys, (key) => {
                if (!_.includes(mainDataKeyStack, key)) {
                  const { respuestaId } =
                    _.find(oportunidad.detalle, (d) => d.preguntaId == key) ||
                    {};
    ...
    if (
      Object.keys(preguntas || {}).length !==
        Object.keys(nextProps.preguntas || {}).length &&
      nextProps.primeraRespuesta
    ) {
      this.handleColumnsStateMap(
        nextProps.preguntas || {},
        preguntas || {},
        nextProps.primeraRespuesta
      );
    ...
    if (
      this.props.empresas !== nextProps.empresas ||
      _.size(this.props.empresas) == _.size(nextProps.empresas)
    ) {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    Object.keys(this.requiredColumns || {}).map((key) => {
      Object.keys(nextState.form || {}).map((field) => {
        if (
          this.requiredColumns[key] === field &&
          _.isNull(nextState.form[field])
        ) {
          validationCount++;
        }
      });
    ...
    Object.keys(this.requiredDefaultColumns || {}).map((key) => {
      Object.keys(nextState.form || {}).map((field) => {
        if (
          this.requiredDefaultColumns[key] === field &&
          _.isNull(nextState.form[field])
        ) {
          validationCount++;
        }
  ...
  componentDidUpdate(prevProps, prevState) {
    if (!this.state.backupLoaded) {
      ...
      const backupLoaded =
        estadoId === 1
          ...
          : !!oportunidadId &&
            ...
            (backupDataLoadedCount + _.size(preguntas) === initialDetalleSize ||
      ...
      const {
        actions: { sobrescribirAtributosFormOportunidad },
        primeraPregunta,
      } = this.props;
      const {
        form: { estadoId },
  ...
  setOportunidadFormEdition = (oportunidad) => {
    const {
      actions: { obtenerFormOportunidad, actualizarDetalleOportunidad },
      preguntas,
      primeraPregunta,
    } = this.props;
    const {
  ...
  getProducto = (preguntas, respuestas, ProductoNombre) => {
    const k = _.find(
      _.keys(preguntas),
      (k) => preguntas[k].name === "Producto"
    );
    const res = _.find(
      _.keys(respuestas[k]),
      (k2) => respuestas[k][k2].name === ProductoNombre
    );
  ...
  handleColumnsStateMap = (values, oldValues, respuestaId) => {
    ...
    _.map(_.values(oldValues), (oldValue) => {
      if (
        !_.find(_.values(values), (newValue) => oldValue.name === newValue.name)
      ) {
    ...
    if (_.size(_.keys(values)) === 1) {
      ...
      this.setState({
        form: {
          ...form,
          [atributo]: respuestaId,
          [`${atributo}Nombre`]: valueNombre,
        },
      });
    } else {
    ...
    this.setState({
      ...stateParams,
      preguntasModificadas: false,
      onPressBooleanButton: false,
    });
    if (estadoId !== 1)
      this.setState({ form: { ...form, ...this.state.backup.form } });
  };
  ...
  onSetPrimeraRespuesta = (value) => {
    const {
      ...
      respuestas,
      actualPregunta,
      actions: { obtenerPreguntasFormOportunidad, limpiarForm },
    } = this.props;
    const {
      backup,
      form: { estadoId },
  ...
  onCreateSummaryOportunidad = () => {
    const { currentUser, oportunidadId, colaboradores, oportunidad } =
      this.props;
    ...
    const plataformEmpresa =
      plataform === undefined ||
      plataform[this.state.form.rutCliente] === undefined ||
      plataform[this.state.form.rutCliente].plataforma === undefined ||
      plataform[this.state.form.rutCliente].plataforma === ""
        ? null
        : plataform[this.state.form.rutCliente].plataforma;

    usuarioResponsable = _.find(
      colaboradores,
      (pt) => pt.usuarioNt === form["usuarioNTResponsable"]
    ) || {
      nombreColaborador: currentUser.nombreCompleto,
      usuarioNt: currentUser.usuario,
      centroCosto: currentUser.plataforma,
    ...
    //editar oportunidad
      ...
      usuarioResponsable.centroCosto =
        usuarioResponsable.centroCosto === undefined
          ? oportunidad.plataformaResponsable
          : usuarioResponsable.centroCosto;
    ...
    const plataformaCreador =
      oportunidadId === null
        ? currentUser.plataforma
        : oportunidad.plataformaCreador;
    ...
    Object.keys(form).map((st) => {
      Object.keys(preguntas || {}).map((key) => {
        ...
        if (st === preguntas[key].name) {
          ...
          if (
            st.toLowerCase() === "negocio" ||
            st.toLowerCase() === "producto"
          ) {
          ...
          detalle.push({
            ...
            ...(_.includes(
              [TipoPregunta.LIBRE, TipoPregunta.ESTRUCTURADO],
              tipo
            )
              ? { respuesta: form[st] }
    ...
    Object.assign(paramsForm, {
      monto:
        tieneMonto && form["Monto"]
          ? Number(form["Monto"].replace(/\./g, ""))
          : 0,
    });
    ...
    if (oportunidad.resumen !== undefined) {
      Object.assign(resumen, {
        fechaCreacion: oportunidad.resumen.fechaCreacion,
      });
    }
    ...
    Object.assign(paramsForm, {
      notas: _.compact(
        _.map(form["notas"], (nota) =>
          nota.contenido
            ? {
                ...nota,
                fechaCreacion:
                  nota.fechaCreacion == "" ? fechaCreacion : nota.fechaCreacion,
              }
            : null
        )
      ),
  ...
  onSaveOportunidad = () => {
    const {
      actions: { crearOportunidad, actualizarOportunidad },
      oportunidadId,
      fromVisita,
      addToVisita,
      navigation,
  ...
  onPressEstadoOportunidad = (estado) => {
    const {
      actions: { obtenerFormOportunidad },
      estadosOportunidad,
      preguntas,
      respuestas,
    } = this.props;
  ...
  validationForm = (validarRequeridos = false) => {
    ...
    if (
      _.isNull(form["usuarioNTResponsable"]) ||
      _.isEmpty(form["usuarioNTResponsable"])
    )
      validation++;
    if (_.isNull(form["rutCliente"]) || _.isEmpty(form["rutCliente"]))
      validation++;
    if (
      _.isEmpty(form["notas"]) ||
      !_.find(form["notas"], (nota) => !!nota.contenido.trim())
    )
      validation++;
    if (estadoId === 2) {
      if (
  ...
  renderAlertModal = () => {
    ...
    return (
      ...
      >
        <Text style={styles.modalTitle}>Oportunidad guardada exitosamente</Text>
        ...
        <View style={styles.modalContainerButtons}>
          ...
          <TouchableOpacity
            onPress={() => this.goNuevaOportunidad()}
            style={[styles.buttonModal, styles.buttonSolid]}
          >
    ...
    const respuesta =
      title === "responsable"
        ...
        : (
            _.find(
              colaboradores,
              (responsable) => responsable.usuarioNt === form[title]
            ) || {}
          ).nombreColaborador || form[title];
    const modalVisibleKey = `${value ? title : "responsable"}ModalVisible`;
    const modalVisible =
      this.state[`${value ? title : "responsable"}ModalVisible`];
    const requeridoFaltante = requeridosFaltantes && !respuesta;
    const inputStyle = [styles.inputContainer].concat(
      this.isEditableFormAttribute(title) ? [] : styles.inputContainerDisabled
    );
    ...
    return (
      <View key={value}>
        <View style={inputStyle}>
          ...
          <TouchableOpacity
            onPress={() =>
              this.isEditableFormAttribute(title)
                ? this.setState({ [modalVisibleKey]: true })
                : null
            }
          >
            {respuesta ? (
              <Text
                style={[styles.inputTitle].concat(
                  requeridoFaltante ? styles.inputTitleRequired : []
                )}
              >{`${capitalizedTitle} oportunidad*:`}</Text>
            ) : null}
            <View style={styles.itemContainer}>
              <Text style={styles.inputText}>
                {respuesta
                  ? respuesta
                  : `${capitalizedTitle} ${value ? "" : "oportunidad"}*`}
              </Text>
              <Image
  ...
  renderRutClienteInput = () => {
    const {
      ...
      form: { estadoId, rutCliente },
      requeridosFaltantes,
    } = this.state;
    const requeridoFaltante = requeridosFaltantes && !rutCliente;
    const inputStyle = [styles.inputContainer].concat(
      this.isEditableFormAttribute("rutCliente")
        ? []
  ...
  renderGrupoEconomicoInput = () => {
    const {
      ...
      form: { estadoId, grupoEconomico },
    } = this.state;
    return (
      <View
        style={[styles.inputContainer].concat(
          this.isEditableFormAttribute("grupoEconomico")
            ? []
            : styles.inputContainerDisabled
        )}
      >
        ...
        >
          {grupoEconomico ? (
            <Text style={styles.inputTitle}>Grupo económico*:</Text>
          ) : null}
          <View style={styles.itemContainer}>
            <Text style={styles.inputText}>
  ...
  renderPrimeraPreguntaInput = (srcProps, pID) => {
    ...
    let title =
      preguntas[actualPregunta] !== undefined
        ? preguntas[actualPregunta].name
        : "Cargando...";
    let list =
      respuestas[actualPregunta] !== undefined
        ? respuestas[actualPregunta]
        : [];
    let obligatorio = !!(
      preguntas[actualPregunta] !== undefined &&
      preguntas[actualPregunta].obligatorio
    );
    const requeridoFaltante = obligatorio && requeridosFaltantes && !value;
    const inputStyle = [styles.inputContainer].concat(
      this.isEditableFormAttribute(atributo)
        ? []
        : styles.inputContainerDisabled
    );
      ...
      <View>
        <View style={inputStyle}>
          ...
          <TouchableOpacity
            onPress={() =>
              this.isEditableFormAttribute(atributo)
                ? this.setState({ [`${atributo}Modal`]: true })
                : null
            }
          >
            {valueNombre ? (
      ...
      return (
        <View key={key}>
          <View style={inputStyle}>
            ...
            <TouchableOpacity
              onPress={() => this.setState({ [tituloModal]: true })}
              disabled={!isEditable}
            >
  ...
  renderLibreInput = (value, srcProps, pID) => {
    ...
    let title =
      preguntas[value] !== undefined ? preguntas[value].name : "Cargando...";

    let icon =
      preguntas[value].tipo === TipoPregunta.MONEDA ? (
        <Image
          source={require("./../../assets/images/icons/monetization_input_icon.png")}
        />
      ) : (
        []
      );
    let obligatorio =
      !!(preguntas[value] !== undefined && preguntas[value].obligatorio) ||
      (estadoId === 2 && title === "Spread");

    const respuesta = form[title];
    const enteroInvalido =
      preguntas[value].tipo === TipoPregunta.MONEDA ||
      preguntas[value].tipo === TipoPregunta.ENTERO
        ? !!respuesta && invalidInteger(respuesta)
        : false;
    const decimalInvalido =
      preguntas[value].tipo === TipoPregunta.DECIMAL
        ? !!respuesta && invalidDecimal(respuesta)
        : false;
    const requeridoFaltante =
      (obligatorio && requeridosFaltantes && !respuesta) ||
      enteroInvalido ||
      decimalInvalido;
    const inputStyle = [styles.inputContainer].concat(
      this.isEditableFormAttribute(title) ? [] : styles.inputContainerDisabled
      ...
      <View key={value}>
        <View style={inputStyle}>
          ...
          <InputGroup
            ...
            keyboardType={
              _.includes(
                [TipoPregunta.MONEDA, TipoPregunta.ENTERO],
                preguntas[value].tipo
              )
                ? "number-pad"
                : preguntas[value].tipo === TipoPregunta.DECIMAL
                ? "decimal-pad"
            ...
            leftContent={icon}
            value={
              TipoPregunta.MONEDA === preguntas[value].tipo ||
              TipoPregunta.ENTERO === preguntas[value].tipo
                ? montoPuntosMil(this.state.form[title])
        ...
        {requeridoFaltante ? (
          <Text style={styles.requeridoMssg}>
            {!enteroInvalido && !decimalInvalido
              ? "campo requerido"
              : enteroInvalido
              ? "campo debe ser un número entero"
              : "campo debe ser un número decimal"}
          </Text>
        ) : null}
      </View>
    );
  ...
  renderFormColumns = (fromBackup = false) => {
    const srcProps = fromBackup
      ? {
          ...
          respuestas:
            this.state.backup.respuestas || this.props.backup.respuestas,
        }
      : this.props;
    ...
    return (
      <View>
        {this.renderPrimeraPreguntaInput(
          srcProps,
          `P${this.testIDCount}OPORTUNIDAD`
        )}

        {preguntasLength > 1
          ? _.map(preguntas[actualPregunta].childrensId, (chId) => {
              this.testIDCount += 1;
              return this.renderFormInputWithChildrens(
                { ...preguntas[chId], parentId: false },
                chId,
                srcProps
              );
            })
        ...
        >
          ...
          >
            <Text style={styles.buttonText}>
              {fromVisita ? "Agregar a visita" : "Guardar"}
            </Text>
    ...
    if (oportunidadId) {
      return {
        ...
        fechaCreacion: moment(
          oportunidad.resumen.fechaCreacion,
          "YYYY-MM-DD HH:mm:ss Z"
        )
  ...
  ultimoCambioOportunidad = () => {
    ...
    if (oportunidadId && oportunidad.ultimoCambio !== null) {
      return {
        ...
        fechaEdicion: moment(oportunidad.ultimoCambio.fechaCreacion).format(
          "YYYY-MM-DD HH:mm:ss"
        ),
      };
    } else {
  ...
  renderForm = () => {
    ...
    const {
      form: { estadoId, rutCliente, nombreCliente, notas },
      loadingTab,
      requeridosFaltantes,
    } = this.state;
    ...
    if (requeridosFaltantes && notaFaltante)
      notaTitleStyle.push(styles.inputTitleRequired);
    ...
    return (
      <View style={styles.container}>
        ...
        >
          <ScrollView style={styles.formContainer}>
            <View style={styles.userContainer}>
              ...
              <Text style={styles.dateHeader}>{fechaCreacion}</Text>
            </View>
            ...
            {oportunidadId && ultimoCambio && (
              <View style={styles.userContainer}>
                ...
                <Text style={styles.creator}>{ultimoCambio.usuarioNT}</Text>
            ...
            {oportunidadId ? (
              <EstadosOportunidad
                value={
                  (_.find(estadosOportunidad, (e) => e.id === estadoId) || {})
                    .nombre
                }
                estadoId={oportunidad.estadoId}
                editable={
                  (oportunidad.estadoId === 1 && estadoId !== 1) || editable
                }
                changeTabValue={
                  oportunidad.estadoId === 1
                    ? this.onPressEstadoOportunidad
                    : () => null
                }
              />
            ) : null}
            ...
            {loadingTab ? (
              ...
              <View>
                ...
                <View
                  style={
                    rutCliente
                      ? [styles.inputContainer].concat(
                          estadoId !== 2 ? [] : styles.inputContainerDisabled
                        )
                ...
                <Text style={notaTitleStyle}>{"Nota(s) *."}</Text>
                {requeridosFaltantes && notaFaltante ? (
                  <Text style={styles.requeridoMssg}>
                    debe haber al menos una nota
                  </Text>
                ) : null}
        ...
        {loadingTab || (oportunidadId && oportunidad.estadoId !== 1) ? null : (
          <TouchableOpacity
            onPress={() => {
              this.validationForm(true)
                ? this.onCreateSummaryOportunidad()
                : false;
            }}
            style={
              editable
          ...
          >
            <Text style={styles.buttonText}>
              {oportunidadId ? "Editar" : fromVisita ? "Agregar" : "Crear"}
            </Text>
            {isFetchingObtenerPregunta && (
  ...
  render() {
    ...
    return error === "unauthorized" ? (
      ...
      <Text>
        {
          "Ha habido un error en el sistema. Por favor vuelva intentarlo mas tarde."
        }
      </Text>
    ) : (
      this.renderForm()
