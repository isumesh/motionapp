...
class FiltroOportunidadScreen extends Component<Props> {
  ...
  componentWillMount() {
    ...
    const estadoNombre = estadoId
      ? {
          estadoNombre: _.find(estadosOportunidad, (e) => e.id === estadoId)
            .nombre,
        }
      : {};
  ...
  componentWillReceiveProps(nextProps) {
    const {
      ...
      navigation: { navigate },
      navigation,
      actions: { updateOportunidadFilterParams },
    } = this.props;
  ...
  getFiltersParams = () => {
    const {
      preguntas,
      currentUser: { plataforma, usuario },
    } = this.props;
    const { form } = this.state;
    let filterParams = {};
    let respuestas = [];
    ...
    Object.assign(filterParams, {
      nombreCliente:
        form["nombreCliente"] === undefined ? "" : form["nombreCliente"],
    });
    Object.assign(filterParams, {
      respuestasId: _.isEmpty(respuestas)
        ? form.respuestasId || []
        : respuestas,
    });
    ...
    Object.assign(filterParams, {
      macrobancaEmpresa: form["macrobancaEmpresa"],
    });
    Object.assign(filterParams, {
      plataformaEmpresa: form["obtenerPlataform"],
    });
  ...
  renderResponsable = () => {
    ...
    return (
      <View style={styles.inputContainer}>
        ...
        <TouchableOpacity
          onPress={() => this.setState({ responsableModalVisible: true })}
        >
          {responsable ? (
            <Text style={styles.inputTitle}>Ejecutivo responsable:</Text>
          ) : null}
          <View style={styles.itemContainer}>
            <Text style={styles.inputText}>
              {responsable ? responsable : "Ejecutivo responsable"}
            </Text>
            <Image
        ...
        >
          ...
          >
            ...
            <View style={styles.itemContainer}>
              <Text style={styles.inputText}>
                {from_date
                  ? moment(from_date).format("dddd, DD MMMM YYYY")
                  : "Desde"}
              </Text>
              <Image
                style={{ marginRight: 10 }}
            ...
            <View style={styles.itemContainer}>
              ...
              <Text
                style={[styles.inputText].concat(
                  formValidation["end_date"] ? styles.inputTitleRequired : []
                )}
              >
                {end_date
                  ? moment(end_date).format("dddd, DD MMMM YYYY")
                  : "Hasta"}
              </Text>
              <Image
                style={{ marginRight: 10 }}
  ...
  renderEstadoInput = () => {
    ...
    Object.keys(estadosOportunidad || {}).map((key) => {
      Object.assign(estados, {
        [estadosOportunidad[key].id]: { name: estadosOportunidad[key].nombre },
      });
    });
    ...
    return (
      <View style={styles.inputContainer}>
        ...
        <TouchableOpacity
          onPress={() => this.setState({ estadoModalVisible: true })}
        >
          <Text style={styles.inputTitle}>Estado</Text>

          <View style={styles.itemContainer}>
            <Text style={styles.inputText}>
              {estadoNombre ? estadoNombre : "Activa"}
            </Text>
            <Image
              style={{ marginRight: 10 }}
              source={require("./../../assets/images/icons/down_arrow_icon.png")}
  ...
  renderMacrobanca = () => {
    const {
      macroBancaModalVisible,
      form: { macrobancaEmpresa },
    } = this.state;
    const {
      macroBancas: { lista },
    } = this.props;
    return (
      <View style={styles.inputContainer}>
        <Modal
          animationType={"slide"}
          transparent={false}
          visible={macroBancaModalVisible}
          onRequestClose={() => {}}
        >
          <ItemListModal
            list={lista}
        ...
        <TouchableOpacity
          onPress={() => this.setState({ macroBancaModalVisible: true })}
        >
          {macrobancaEmpresa ? (
            <Text style={styles.inputTitle}>Macrobanca:</Text>
          ) : null}
          <View style={styles.itemContainer}>
            <Text style={styles.inputText}>
              {macrobancaEmpresa ? macrobancaEmpresa : "Macrobanca"}
            </Text>
            <Image
              style={{ marginRight: 10 }}
              source={require("./../../assets/images/icons/down_arrow_icon.png")}
  ...
  renderObtenerPlataforma = () => {
    const {
      obtenerPlataformaModalVisible,
      form: { obtenerPlataform },
    } = this.state;
    const {
      obtenerPlataforma: { lista },
    } = this.props;
    return (
      <View style={styles.inputContainerPlatforma}>
        <Modal
          animationType={"slide"}
          transparent={false}
          visible={obtenerPlataformaModalVisible}
          onRequestClose={() => {}}
        >
          <ItemListModal
            list={lista}
        ...
        <TouchableOpacity
          onPress={() => this.setState({ obtenerPlataformaModalVisible: true })}
        >
          {obtenerPlataform ? (
          ...
          <View style={styles.itemContainer}>
            <Text style={styles.inputText}>
              {obtenerPlataform ? obtenerPlataform : "Plataforma"}
            </Text>
            <Image
              style={{ marginRight: 10 }}
              source={require("./../../assets/images/icons/down_arrow_icon.png")}
